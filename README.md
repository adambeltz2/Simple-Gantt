...
![Version](https://img.shields.io/badge/version-2.42.0-blue) ![License](https://img.shields.io/badge/license-MIT-green) [![Buy Me a Coffee](https://img.shields.io/badge/Buy%20me%20a%20coffee-☕-FFDD00)](https://buymeacoffee.com/adambeltz)
...
- **Plan My Day** – A "Plan My Day" toolbar button opens a docked panel over the grid/chart (header and toolbar stay usable) with a "Recommended" list computed live from Late/upcoming tasks -- overdue first, then due today, then due soon, capped so it never becomes a second copy of the grid. Pin whichever ones matter into "Today's Plan" (from the panel's "+", or the ★ toggle on any row's Task ID in the grid, both stay in sync); check one off to mark it 100% done, or click its 📝/+ icon to open that task's real Notes right there. A "Start New Day" link clears the pin list in one click once you're ready to start curating fresh, rather than yesterday's picks rolling forward silently. Per-project, localStorage-only, never in CSV export or Dropbox backup
- **Task Notes** – Every task has a built-in Notes column (right after Labels) -- no setup needed. Click its flag, or right-click the row and choose "Add Notes"/"View/Edit Notes," for a small click-to-expand modal rendering a lightweight Markdown subset (headers, bold, italic, links, lists, task checkboxes, inline `code` and fenced ` ``` ` code blocks); still a plain-text cell underneath, so it round-trips through CSV import/export like any other column. (A custom column named "Notes" still gets the same treatment too, for anyone using that older convention.)
- **Project Notes** – A single Markdown note for the whole project, separate from any one task -- scope, context, a changelog of decisions. Opened via the toolbar's "Notes" button, using the same click-to-expand-and-edit Markdown UX (and renderer) as the per-task Notes column. Stored with the project alongside the named-resources registry and collapse state: localStorage-only, not part of CSV export -- but is included in a Dropbox backup's `meta.json`, and restored back out along with the rest of that backup
...
- **Global Clear Filters** – A "Clear filters" button in the toolbar (next to Search/Labels/Filters) resets Grid Search, the Label filter, and the Structured Filters dropdown all in one click, rather than clearing each of the three individually. Disabled whenever none of them has anything active
- **Labels** – Tag any row with one or more free-text labels (comma or semicolon-separated). The toolbar's label filter is a checkbox multi-select, sourced from your named labels: check one or more to narrow the grid to tasks carrying any of them; an opt-in "Label in chart" toggle applies that same filter to the Gantt chart
...
- **Named Labels** – Same idea as Named Resources, one-to-one: define labels ahead of time via the "Labels" toolbar button, then pick them from a quick-pick ▾ icon on any grid Labels cell. The Labels cell stays free text underneath -- the registry is purely structural, no color/badge treatment. Renaming a registered label updates every task already carrying it; deleting one only affects future suggestions
