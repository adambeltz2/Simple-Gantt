...
7. ~~PDF export~~ -- **done in v2.15.0.** (Landscape-Letter paginated export via a new jsPDF dependency, built on the same html2canvas rasterization already used by PNG export -- the single captured canvas is sliced into a grid of pages, each labeled with the project name, date, and a "page X of Y (row/col)" footer so a multi-page printout can be reassembled. A >40-page result -- e.g. a multi-year plan at Day zoom -- prompts for confirmation before generating rather than silently producing a huge file.)
