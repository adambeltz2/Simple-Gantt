...
- **Dropbox backup retention -- confirm the existing cap matches what you want.** `MAX_BACKUPS_PER_PROJECT` in `index.html` already caps backups at 25 per project and auto-deletes the oldest once that's exceeded (`pruneOldBackups()`). If 100 is the number you actually want, this is a one-line change; otherwise this item is already done.
