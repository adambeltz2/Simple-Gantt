...
12. **Bulk edit / multi-row select** -- select multiple rows and apply a Resource/%Done/Parent change to all of them at once. Needs figuring out jexcel's selection-range API and a clear UX; may partially already be possible via existing copy/paste-down-a-column behavior, worth checking before building new UI.
